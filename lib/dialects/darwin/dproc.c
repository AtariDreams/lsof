...
#    define FILEPORTS_INCR                                                     \
        (sizeof(struct proc_fileportinfo) * 32) /* Fileports space increment   \
                                                 */
    ...
    for (ef = 0; !ef;) {
        ...
        if ((nb + sizeof(struct proc_fileportinfo)) < NbFps) {
            ...
            if (!Fps)
                ...
                Fps = (struct proc_fileportinfo *)realloc((MALLOC_P *)Fps,
                                                          (MALLOC_S)NbFps);
        }
    }
                         ...
                         int pid)                  /* PID */
{
    ...
    for (a = (uint64_t)0, i = n = 0; i < 10000; i++) {
        nb = proc_pidinfo(pid, PROC_PIDREGIONPATHINFO, a, &rwpi, sizeof(rwpi));
        if (nb <= 0) {
